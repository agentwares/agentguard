 ...
 */
import { estimateLlmCost, type LlmUsage } from "@agentwares/agentguard-core";
  ...
  return async (input, init) => {
    ...
    const charge = async (usage: LlmUsage | undefined, resolvedModel: string): Promise<void> => {
      ...
      await ag.spend(usd ?? 0, `llm:${resolvedModel}`, {
        ...
        reason:
          usd === undefined
            ? `no list price for "${resolvedModel}" — not counted against spend_usd; add it under spend.models in agentguard.yaml`
            : undefined,
      });
    };
