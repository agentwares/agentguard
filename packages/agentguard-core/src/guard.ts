...
export class Guard {
  ...
  async run(call: GuardCall, execute: Execute): Promise<GuardResult> {
    ...
    // Spend-class calls always declare `spend_usd`, even at 0. An unpriced spend tool would
    // otherwise never be measured against the cap, so a run whose dollars are only known from
    // results could keep spending for ever once it was over budget.
    if (cls === "spend") delta.spend_usd = usdEstimate ?? 0;
    else if (usdEstimate !== undefined && usdEstimate > 0) delta.spend_usd = usdEstimate;
...
function capError(check: CapCheck, runId: string): GuardErrorBody {
  ...
  // `attempted === used` means the counter was already over the limit before this call — money
  // that landed after the fact (result-priced tools, LLM token usage), not this call's estimate.
  const cause =
    x.attempted === x.used
      ? `${x.counter} cap for ${scope} is ${unit}${x.limit} and ${unit}${x.used} is already used`
      : `${x.counter} cap for ${scope} is ${unit}${x.limit}; used ${unit}${x.used}, this call would make it ${unit}${x.attempted}`;
  return {
    code: "CAP_EXCEEDED",
    cause,
    fix:
